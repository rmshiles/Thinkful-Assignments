...
--3.
SELECT state, COUNT(avg_math_4_score) AS count_of_scores, AVG(avg_math_4_score)AS avg_of_scores,
...
--6. 
SELECT state, ROUND(AVG(avg_math_4_score),2 )AS avg_state_math 
FROM naep
WHERE year = 2000 
GROUP BY state
ORDER BY state;
...
ORDER BY total_expenditure; --naep.state;
